...
class MARCo(nn.Module):
    ...
    def encode_image(self, imgs):
        ...
        radar_enc = self.radar_encoder(
            imgs=radar_imgs,
            attn_bias=attn_bias,
            mask_info=None
        )

        optical_enc = self.optical_encoder(
            imgs=optical_imgs,
            attn_bias=attn_bias,
            mask_info=None
        )

        joint_enc = self.cross_encoder(
            x=radar_enc,
            context=optical_enc,
            alibi=attn_bias
        )

        z = joint_enc.mean(dim=1)  
        return z
    ...
    def forward(self, imgs, radar_mask_info, optical_mask_info, rank=0, world_size=1):
        ...
        optical_masked_attn_bias = apply_mask_to_alibi(
            ...
            batch_size=radar_imgs.shape[0],
            orig_seq_len=self.num_patches,
            q_len=optical_mask_info['len_keep'],
...
class BaseTransformer(nn.Module):
    def __init__(self, dim, layers, attention_heads=8, ff_mult=4, final_norm=True):
        super().__init__()
...
class BaseTransformerCrossAttn(nn.Module):
    def __init__(self, dim, layers, attention_heads=8, ff_mult=4):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(layers):
            self.layers.append(nn.ModuleList([
...
def apply_mask_to_alibi(
    ...
    alibi = torch.gather(
        alibi,
        dim=2,
        index=ids_keep_queries[:, None, :, None].repeat(
            1, attention_heads, 1, orig_seq_len
        )
    )
    ...
    alibi = torch.gather(
        alibi,
        dim=3,
        index=ids_keep_keys[:, None, None, :].repeat(
            1, attention_heads, q_len, 1
        )
    )
...
def gather_features(features, world_size):
    if world_size == 1 or not dist.is_available() or not dist.is_initialized():
        return features
    gathered_features = [
        torch.zeros_like(features) for _ in range(world_size)
    ]
...
class ContrastLossInput(nn.Module):
    def __init__(self, projection_input=768, projection_output=768):
        super().__init__()
...
class HybridPatchEmbed(nn.Module):
    def __init__(self, in_chans=3, embed_dim=768):
        ...
        self.conv_stem = nn.Sequential(
            nn.Conv2d(in_chans, embed_dim // 2, 3, stride=2, padding=1),
...
class ViT(nn.Module):
    def __init__(self, num_patches, dim=768, layers=12, attention_heads=16, in_channels=12, patch_size=8):
        super().__init__()
        self.dim = dim
        self.layers = layers
        self.attention_heads = attention_heads
        ...
        pos_embed = get_2d_sincos_pos_embed(dim, int(num_patches ** 0.5))
...
class DecoderMAE(nn.Module):
    def __init__(self, num_patches, encoder_dim=768, decoder_dim=768, decoder_layers=12, attention_heads=16, 
                 total_channels=14, patch_size=8):
        super().__init__()
        self.decoder_dim = decoder_dim
        self.decoder_layers = decoder_layers
        self.attention_heads = attention_heads
    ...
    def forward(self, x, mask_info_radar, mask_info_optical, target):
        ...
        pred = rearrange(x, 'b (h w) (c i j) -> b c (h i) (w j)', c=14, i=8, j=8, h=12, w=12)
        ...
        target = rearrange(target, 'b (h w) (c i j) -> b c (h i) (w j)', c=14, i=8, j=8, h=12, w=12)
        ...
        loss_optical = loss_optical.mean(dim=-1)
        loss_optical = (loss_optical * mask_info_optical['mask_for_mae']).sum() / mask_info_optical['mask_for_mae'].sum()

        loss_radar = (pred_radar - target_radar) ** 2
        loss_radar = loss_radar.mean(dim=-1)
        loss_radar = (loss_radar * mask_info_radar['mask_for_mae']).sum() / mask_info_radar['mask_for_mae'].sum()
